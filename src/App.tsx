...
type PanelView = 'models' | 'settings' | null
...
function App() {
  ...
  return (
    <main className="app-shell" style={{ '--background-image': `url(${backgroundImage})` } as CSSProperties}>
      ...
      <section className="floating-layout">
        ...
        <header className="floating-topbar glass-card">
          <div className="topbar-brand">
            <h2>OpenFlow</h2>
            <span>{settings.displayName || 'Pilot'}</span>
          </div>
          <nav className="topbar-nav">
            <button
              className={panelView === 'models' ? 'ghost-button nav-active' : 'ghost-button'}
              type="button"
              onClick={() => setPanelView(panelView === 'models' ? null : 'models')}
            >
              Models
            </button>
            <button
              className={panelView === 'settings' ? 'ghost-button nav-active' : 'ghost-button'}
              type="button"
              onClick={() => setPanelView(panelView === 'settings' ? null : 'settings')}
            >
              Settings
            </button>
          </nav>
        ...
        <section className="floating-chat glass-card">
          <div className="chat-head-minimal">
            <div>
              <span className="eyebrow">AI Chat</span>
              <h3>{activeModel.label}</h3>
            </div>
            <span className="model-badge">{activeModel.openRouterModel}</span>
          </div>

          <div className="chat-log">
            {!activeThread || activeThread.messages.length === 0 ? (
              <div className="empty-chat">
                <p>Start chatting. Your recent chats are in the left edge drawer.</p>
              </div>
            ) : (
        ...
        {panelView === 'models' ? (
          <section className="floating-panel glass-card">
            <div className="panel-head">
              <div>
                <span className="eyebrow">Models</span>
                <h4>Select active model</h4>
              </div>
            ...
            <div className="model-switcher">
              {MODEL_OPTIONS.map((model) => (
                <button
                  key={model.id}
                  className={model.id === activeModel.id ? 'model-pill model-pill--active' : 'model-pill'}
                  type="button"
                  onClick={() => {
                ...
                >
                  ...
                  <span>{model.openRouterModel}</span>
